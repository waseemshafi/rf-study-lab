...
const FIG = (function () {
  ...
  const map = {
    ...
    'dsss-receiver-design': [
      { type: 'acqSearch', title: 'Stage 1 — acquisition', caption: 'The receiver first slides the local code to find the correlation peak (coarse lock).', explain: EX('<b>What it shows:</b> the front of the receiver chain — a search over code phase that must clear the threshold before tracking can start. <b>Try:</b> lower the SNR and the peak sinks; longer dwell (more integration) buys it back — the acquisition sensitivity/time trade the design must budget.') },
      { type: 'earlyLate', title: 'Stage 2 — code tracking (DLL)', caption: 'After handover, the Early−Late discriminator keeps the prompt correlator on the peak.', explain: EX('<b>What it shows:</b> the closed-loop code tracker the design runs concurrently with a carrier loop. <b>Try:</b> drag the error — the S-curve slews the code NCO back to alignment so the prompt arm keeps feeding clean symbols to the demodulator.') },
      { type: 'despreadCollapse', title: 'Stage 3 — despread & margin', caption: 'The prompt replica collapses the signal narrowband and spreads any jammer thin.', explain: EX('<b>What it shows:</b> the payoff stage — despreading lifts the signal by the processing gain Gp and buries a jammer, which is exactly the jamming margin the receiver is designed around. <b>Try:</b> raise Gp to widen the margin (at the cost of more bandwidth and a longer acquisition search).') }
    ],
